...
namespace Compress
{
    ...
    {
        List<PointF[]> PointsXY = new List<PointF[]>();//存放读取的平面坐标数据
        List<PointF[]> PointsBL = new List<PointF[]>();//存放兰伯特投影反解经纬度数据
        ...
        public static List<PointF[]> readMapPoints(string fileName, out List<int> id)        //获取地图数据里的点
        {
            ...
            for (int i = 0; i < lines.Length; i++)
            {
                ...
                {
                    mapPoints.Add(points.ToArray());
                    points.Clear();
                    if (lines[i + 1].ToUpper() == "END")//读到两个END，结束
                        break;
                    else
                    {
                        id.Add(int.Parse(lines[i + 1].Trim()));
                        i++;
                    }
                }
                else
                {
                    ...
                    point.Y = Convert.ToSingle(data[1]);
                    points.Add(point);
                }
            }
            return mapPoints;
        ...
        static void computeABC(out double A,out double B,out double C,PointF M,PointF N)
        {
            ...
            double temp = Math.Sqrt(Math.Pow(yM - yN, 2) + Math.Pow(xM - xN, 2));
            A = (yM - yN) / temp;
            B = (xN - xM) / temp;
            C = (xM * yN - xN * yM) / temp;
        }
        ...
        static void f(PointF[] points,List<PointF> compressedPoints ,int M,int N,double threshold)
        {
            ...
            if (pointsCount>=3)
            {
                computeABC(out A, out B, out C, points[M], points[N]);
        ...
        public static PointF[] DouglasPuckerCompress(PointF[] points,double threshold)
        {
            ...
            f(points, compressedPoints, 0, points.Length - 1,threshold);//添加中间点
